...
// import { activeID } from 'store/reducers/menu'; // I think this maps to selectedID in the store
...
const NavGroup = observer(({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  const { drawerOpen, selectedID } = menuStore;
  ...
  const checkOpenForParent = (child: NavItemType[], id: string) => {
    child.forEach((ele: NavItemType) => {
      ...
      if (ele.url === pathname) {
        // dispatch(activeID(id)); // I think this maps to selectedID in the store
        menuStore.setSelectID(id);
      }
    });
  };
  const checkSelectedOnload = (data: NavItemType) => {
    ...
    childrens.forEach((itemCheck: NavItemType) => {
      ...
      if (itemCheck.url === pathname) {
        // dispatch(activeID(currentItem.id!)); // I think this maps to selectedID in the store
        menuStore.setSelectID(currentItem.id!);
      }
    });
  };
